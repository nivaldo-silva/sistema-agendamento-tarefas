...
public class JwtTokenService {
	...
	public LoginResponse generateTokens(Authentication authentication) {
		...
		String accessToken = buildAccessToken(authentication);
	...
	private String buildAccessToken(Authentication authentication) {
		var now = Instant.now();
		...
		if (userId != null) claimsBuilder.claim("user_id", userId);
		if (nome != null) claimsBuilder.claim("nome", nome);

		JwtClaimsSet claims = claimsBuilder.build();
		log.debug("Access token built for: {} with scopes: {}", authentication.getName(), scope);
